...
mod1 <- lmer(log_area_t1 ~ log_area_t0 + avgt0 + (1 | year), data= grow_df )
mod2 <- lmer(log_area_t1 ~ log_area_t0 + avgtm1 + (1 | year), data= grow_df )
mod3 <- lmer(log_area_t1 ~ log_area_t0 + avgtm2 + (1 | year), data= grow_df )

# write it out
bind_rows( fixef(mod1),fixef(mod2), fixef(mod3) ) %>% 
  write.csv(paste0('results/lme4/','grow_',clim_var,'.csv'),row.names=F)
...
# fit_avg <- stan(
#   file = paste0("analysis/stan/grow/gaussian_avg.stan"),
#   data = dat_stan,
...
# 
# write.csv(out_summ, paste0('results/grow_a_summ_',clim_var,'.csv'))
# write.csv(out_post, paste0("results/grow_a_post_",clim_var,".csv"),row.names=F)
# 
# 
...
# 
# write.csv(out_summ, paste0("results/grow_sam_3y_summ_",clim_var,".csv"))
# write.csv(out_post, paste0("results/grow_sam_3y_post_",clim_var,".csv"),row.names=F)


# Average of previous 3 years
fit_avg_re <- stan(
  file = paste0("analysis/stan/grow/gaussian_avg_re.stan"),
  data = dat_stan,
...
write.csv(out_summ, paste0('results/grow_a_summ_re_',clim_var,'.csv'))
write.csv(out_post, paste0("results/grow_a_post_re_",clim_var,".csv"),row.names=F)
...
write.csv(out_summ, paste0("results/grow_sam_3y_summ_re_",clim_var,".csv"))
write.csv(out_post, paste0("results/grow_sam_3y_post_re_",clim_var,".csv"),row.names=F)


# # power exponential moving window
# fit_12_nest <- stan(
#   file = paste0("analysis/stan/grow/gaussian_dirichlet_nest_12.stan"),
#   data = dat_stan,
...
# 
# write.csv(out_summ, paste0("results/grow_sam_12m_summ_",clim_var,".csv"))
# write.csv(out_post, paste0("results/grow_sam_12m_post_",clim_var,".csv"),row.names=F)
...
fit_36_nest <- stan(
  #file = paste0("analysis/stan/grow/gaussian_dirichlet_nest.stan"),
  file = paste0("C:/Users/ac22qawo/gaussian_dirichlet_nest_re.stan"),
  data = dat_stan,
  # pars = c('theta_y', 'theta_m', 'b0', 'b_size', 'b_c'),
  pars = c('theta_y', 'theta_m', 'b_yr', 'b_size', 'b_c', 'b0', 's_yr'),
  warmup = sim_pars$warmup,
  iter = sim_pars$iter,
  thin = sim_pars$thin,
  chains = sim_pars$chains#,
