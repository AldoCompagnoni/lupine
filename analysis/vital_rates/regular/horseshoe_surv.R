...
ppt_mat   <- subset(clim, clim_var == "ppt") %>%
                prism_clim_form("ppt", years, m_back, m_obs)
...
flow_form <- bf(flow_t0 ~ a + b, nl = TRUE) +
                lf(a ~ V1 + V2 + V3 + V4 + V5 + V6 + 
                       V7 + V8 + V9 + V10 + V11 + V12, center = TRUE) +
                lf(b ~ log_area_t0 +
                       (log_area_t0 | year ),
                       # (log_area_t0 | location ) +
                       cmc = FALSE)

mod       <- brm(flow_form, data = flow,
                    family = bernoulli(link = "logit"),
                    prior  = c( prior(horseshoe(1), 
                                      class = "b", 
                                      nlpar = "a") +
                                prior(normal(0, 1), 
                                      class = "b",
...
# save result
# saveRDS( mod, 'results/vital_rates/regular/surv_brms_fit.rds')
...
dat_stan <- list(
  ...
  clim  = select(surv, V1:V12) %>% as.matrix,
  # c_s   = surv$tmp_t0,
  # c_f   = flow$tmp_t0,
  # c_r   = fert$tmp_t0
  hs_df           = 1,   # variance of 
  ...
  hs_scale_global = (2 / (12-2)) / sqrt(12), # global prior scale
...
fit_surv <- stan(
  # file = 'C:/Users/ac22qawo/lupine/surv_ridge.stan',
  file = 'analysis/vital_rates/bayes/stan/surv_ridge.stan',
  data = dat_stan,
  # pars = quote_bare( a_s,
  #                    b_s,
  #                    b_s2,    b_s3),
