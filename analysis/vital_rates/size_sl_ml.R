...
# data format --------------------------------------------------------------
seedl       <- lupine_df %>% 
                  # subset(lupine_df, !is.na(surv_t1) ) %>% 
                  subset( year != 2005 ) %>% 
                  subset( stage_t0 == 'SL' ) %>% 
                  subset( area_t0 != 0) %>% 
...
write.csv(sl_size, 'results/ml_mod_sel/size_sl/size_sl_best_mod.csv', row.names=F)
...
sl_size_clim  <- left_join(seedl, clim_mat) %>%
...
structure_mods <- list(
  ...
  # no quadratic
  log_area_t0 ~ (1 | year),
  log_area_t0 ~ (1 | year) + (1 | location),
  log_area_t0 ~ (1 | location)

)

# fit models
mods <- lapply( structure_mods, function(x) lmer(x, data=sl_size_clim) ) %>% 
          setNames( c( 'yr', 'yr_loc', 'loc') )
...
climate_mods <- list(
  ...
  # precipitation
  log_area_t0 ~  ppt_t0 + (1 | year) + (1 | location),
  log_area_t0 ~  ppt_tm1 + (1 | year) + (1 | location),
  log_area_t0 ~  ppt_t0_tm1 + (1 | year) + (1 | location),
  log_area_t0 ~  ppt_t0_tm2 + (1 | year) + (1 | location),
  
  # temperature
  log_area_t0 ~  tmp_t0 + (1 | year) + (1 | location),
  log_area_t0 ~  tmp_tm1 + (1 | year) + (1 | location),
  log_area_t0 ~  tmp_t0_tm1 + (1 | year) + (1 | location),
  log_area_t0 ~  tmp_t0_tm2 + (1 | year) + (1 | location),
  
  # enso
  log_area_t0 ~  oni_t0 + (1 | year) + (1 | location),
  log_area_t0 ~  oni_tm1 + (1 | year) + (1 | location),
  log_area_t0 ~  oni_t0_tm1 + (1 | year) + (1 | location),
  log_area_t0 ~  oni_t0_tm2 + (1 | year) + (1 | location)
  
)

# fit models
mod_clim <- lapply( climate_mods, function(x) lmer(x, data = sl_size_clim) ) %>% 
              setNames( c( 'null',
...
write.csv(out, 'results/ml_mod_sel/size_sl/size_sl_mod_sel.csv', row.names = F)
...
plot(jitter(surv_t1) ~ log_area_t0, data = sl_clim,
