...
lupine_17 <- read.csv("data/lupine_08_17.csv")
clim      <- read.csv("data/lupine_fc_vars.csv")

# data format --------------------------------------------------------------

# seedling data
seedl_08    <- dplyr::select(lupine_08, location, year, stage_t0, surv_t1)
seedl_17    <- dplyr::select(lupine_17, location, year, stage_t0, surv_t1)
seedl       <- bind_rows(seedl_08, seedl_17)
...
mod1 <- glmer(surv_t1 ~ avgt0 + (1 | year), data= sl_df, 
              family = binomial )
mod2 <- glmer(surv_t1 ~ avgtm1 + (1 | year), data= sl_df, 
              family = binomial )
mod3 <- glmer(surv_t1 ~ avgtm2 + (1 | year), data= sl_df, 
              family = binomial )

# write it out
bind_rows( fixef(mod1),fixef(mod2), fixef(mod3) ) %>% 
  write.csv(paste0('results/lme4/','sl_',clim_var,'.csv'),row.names=F)
...
# fit_avg <- stan(
#   file = paste0("analysis/stan/sl/bernoulli_avg.stan"),
#   data = dat_stan,
#   pars = c('b0', 'b_c'),
...
# 
# write.csv(out_summ, paste0('results/sl_a_summ_',clim_var,'.csv'))
# write.csv(out_post, paste0("results/sl_a_post_",clim_var,".csv"),row.names=F)
# 
# 
# # Multiple years, weighted with simplex
# fit_3yr <- stan(
#   file = paste0("analysis/stan/sl/bernoulli_mYears_simplex.stan"),
#   data = dat_stan,
#   pars = c('theta_k', 'b0', 'b_c'),
#   warmup = sim_pars$warmup,
#   iter = sim_pars$iter,
#   thin = sim_pars$thin,
#   chains = sim_pars$chains#,
...
# Average of previous 3 years
fit_avg_re <- stan(
  file = paste0("analysis/stan/sl/bernoulli_avg_re.stan"),
  data = dat_stan,
  pars = c('b0', 's_yr', 'b_yr', 'b_c'),
  warmup = sim_pars$warmup,
  iter = sim_pars$iter,
  thin = sim_pars$thin,
  chains = sim_pars$chains#,
...
write.csv(out_summ, paste0('results/sl_a_summ_re_',clim_var,'.csv'))
write.csv(out_post, paste0("results/sl_a_post_re_",clim_var,".csv"),row.names=F)


# Multiple years, weighted with simplex
fit_3yr_re <- stan(
  file = paste0("analysis/stan/sl/bernoulli_mYears_simplex_re.stan"),
  data = dat_stan,
  pars = c('theta_k', 'b0', 's_yr', 'b_yr', 'b_c'),
  warmup = sim_pars$warmup,
  iter = sim_pars$iter,
  thin = sim_pars$thin,
  chains = sim_pars$chains#,
...
# # 12 month dirichlet
# fit_12_nest <- stan(
#   file = paste0("analysis/stan/sl/bernoulli_dirichlet_nest_12.stan"),
#   data = dat_stan,
#   pars = c('theta_m', 'b0', 'b_c'),
#   warmup = sim_pars$warmup,
#   iter = sim_pars$iter,
#   thin = sim_pars$thin,
#   chains = sim_pars$chains#,
#   #control = list(adapt_delta = 0.8, stepsize = 0.001, max_treedepth = 20)
# )
# 
# # store results
...
# 
# write.csv(out_summ, paste0('results/sl_sam_12m_summ_,',clim_var,',.csv'))
# write.csv(out_post, paste0('results/sl_sam_12m_post_,',clim_var,',.csv'),row.names=F)


# fit_mean <- stan(
#   file = paste0("analysis/stan/bernoulli_null.stan"),
#   data = dat_stan,
#   pars = c('alpha'),
#   warmup = sim_pars$warmup,
#   iter = sim_pars$iter,
...
# fit_year <- stan(
#   file = paste0("analysis/stan/bernoulli_yr_fixed.stan"),
#   data = dat_stan,
#   pars = c('b_yr', 'log_lik'),
#   warmup = sim_pars$warmup,
#   iter = sim_pars$iter,
...
# fit_36_nest <- stan(
#   file = paste0("analysis/stan/bernoulli_dirichlet_nest.stan"),
#   data = dat_stan,
...
# fit_mYear_beta <- stan(
#   file = paste0("analysis/bernoulli_mYears_beta_site.stan"),
#   data = dat_stan,
#   pars = c('beta', 'beta_site', 'log_lik'),
#   warmup = sim_pars$warmup,
#   iter = sim_pars$iter,
