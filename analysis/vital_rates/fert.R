...
# data format --------------------------------------------------------------
fert        <- subset(lupine_df, !is.na(flow_t1) & flow_t1 == 1 ) %>% 
                  subset( area_t1 != 0) %>% 
...
fert_df   <- fert_clim %>% 
                mutate( year_i = year %>% as.factor %>% as.numeric,
                        ...
                        avgt0   = fert_clim %>% dplyr::select(V1:V12) %>% rowSums,
                        avgtm1  = fert_clim %>% dplyr::select(V13:V24) %>% rowSums,
                        avgtm2  = fert_clim %>% dplyr::select(V25:V36) %>% rowSums )

# climate data
clim_pred <- dplyr::select(fert_df, year) %>%
                inner_join( clim_mat ) %>% 
                unique %>%
                arrange( year ) %>% 
                dplyr::select(-year)
...
# mod1 <- glmmadmb(numall_t1 ~ log_area_t1 + avgt0 + (1 | year), 
#                  data = mutate(fert_df, year = as.factor(year)), family = "nbinom2" )
# mod2 <- glmmadmb(numall_t1 ~ log_area_t1 + avgtm1 + (1 | year), 
#                  data= mutate(fert_df, year = as.factor(year)), family = "nbinom2" )
# mod3 <- glmmadmb(numall_t1 ~ log_area_t1 + avgtm2 + (1 | year), 
#                  data= mutate(fert_df, year = as.factor(year)), family = "nbinom2" )
# 
# # write it out
# bind_rows( fixef(mod1),fixef(mod2), fixef(mod3) ) %>% 
#   write.csv(paste0('results/lme4/','fert_',clim_var,'.csv'),row.names=F)
...
dat_stan <- list(
  n      = nrow(fert_df),
  n_year = fert_df$year_i %>% unique %>% length,
  yr_bck = m_back / 12,
  n_site = fert_df$site_i %>% unique %>% length,
  n_lag  = ncol(clim_pred),
  y      = fert_df$numall_t1,
  x_size = fert_df$log_area_t1,
  clim   = clim_pred,
  clim_means = rowMeans(clim_pred),
  year_i = fert_df$year_i,
  site_i = fert_df$site_i,
  expp_beta = expp_beta,
...
# fit_avg <- stan(
#   file = paste0("analysis/stan/fert/nb_avg.stan"),
#   data = dat_stan,
#   pars = c('b0', 'b_size', 'b_c', 's'),
...
# 
# write.csv(out_summ, paste0('results/fert_a_summ_',clim_var,'.csv'))
# write.csv(out_post, paste0("results/fert_a_post_",clim_var,".csv"),row.names=F)
# 
# 
# # Multiple years, weighted with simplex
# fit_3yr <- stan(
#   file = paste0("analysis/stan/fert/nb_mYears_simplex.stan"),
#   data = dat_stan,
...
# 
# write.csv(out_summ, paste0("results/fert_sam_3y_summ_",clim_var,".csv"))
# write.csv(out_post, paste0("results/fert_sam_3y_post_",clim_var,".csv"),row.names=F)


# Multiple years, weighted with simplex
fit_avg_re <- stan(
  file = paste0("analysis/stan/fert/nb_avg_re.stan"),
  data = dat_stan,
  pars = c('b0', 's_yr', 'b_yr', 'b_size', 'b_c', 's'),
  warmup = sim_pars$warmup,
  iter = sim_pars$iter,
  thin = sim_pars$thin,
  chains = sim_pars$chains#,
...
write.csv(out_summ, paste0('results/fert_a_summ_re_',clim_var,'.csv'))
write.csv(out_post, paste0("results/fert_a_post_re_",clim_var,".csv"),row.names=F)


# Multiple years, weighted with simplex
fit_3yr_re <- stan(
  file = paste0("analysis/stan/fert/nb_mYears_simplex_re.stan"),
  data = dat_stan,
  pars = c('theta_k', 'b0', 's_yr', 'b_yr', 'b_size', 'b_c', 's'),
  warmup = sim_pars$warmup,
  iter = sim_pars$iter,
  thin = sim_pars$thin,
  chains = sim_pars$chains#,
...
write.csv(out_summ, paste0("results/fert_sam_3y_summ_re_",clim_var,".csv"))
write.csv(out_post, paste0("results/fert_sam_3y_post_re_",clim_var,".csv"),row.names=F)
...
# out_post  <- extract(fit_avg_re) %>% as.data.frame
# 
# write.csv(out_summ, paste0('results/fert_a1_summ_re_',clim_var,'.csv'))
# write.csv(out_post, paste0("results/fert_a1_post_re_",clim_var,".csv"),row.names=F)


# # power exponential moving window
# fit_12_nest <- stan(
#   file = paste0("analysis/stan/fert/nb_dirichlet_nest_12.stan"),
#   data = dat_stan,
...
# 
# write.csv(out_summ, paste0("results/fert_sam_12m_summ_",clim_var,".csv"))
# write.csv(out_post, paste0("results/fert_sam_12m_post_",clim_var,".csv"),row.names=F)
...
# # null model
# fit_mean <- stan(
#   file = paste0("analysis/stan/grow/gaussian_null.stan"),
#   data = dat_stan,
#   pars = c('b0', 'b_size', 'sigma'),
#   warmup = sim_pars$warmup,
#   iter = sim_pars$iter,
#   thin = sim_pars$thin,
#   chains = sim_pars$chains#,
...
# fit_year <- stan(
#   file = paste0("analysis/stan/grow/gaussian_ran_yr.stan"),
#   data = dat_stan,
#   pars = c('b0', 'b_size', 'b_yr', 's', 's_yr'),
#   warmup = sim_pars$warmup,
#   iter = sim_pars$iter,
#   thin = sim_pars$thin,
#   chains = sim_pars$chains#,
