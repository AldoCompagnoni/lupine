...
parameters {
  ...
  real b0_size2;
  real<lower=0> s_size2_yr;

  real b_yr[n_year];
  real b_size_yr[n_year];
  real b_size2_yr[n_year];
...
model {
  ...
  b0_size2    ~ normal(0,100);
  s_size2_yr  ~ inv_gamma(0.001, 0.001);
 
  for(ii in 1:n_year){
    b_yr[ii]        ~ normal(b0, s_yr);
    b_size_yr[ii]   ~ normal(b0_size, s_size_yr);
    b_size2_yr[ii]  ~ normal(b0_size2, s_size2_yr);
  }
