...
lupine_cnt  <- read.csv(  paste0(data_dir, "lupine_counts.csv") )
fruit_rac   <- read.csv( paste0(data_dir, "fruits_per_raceme.csv") )
seed_x_fr   <- read.csv( paste0(data_dir, "seedsperfruit.csv") )
pred_g      <- read.csv( paste0(data_dir, "post predation_lupinus tidestromii.csv") )
sl_size     <- read.csv(  paste0(data_dir, "seedl_size.csv") )
clim        <- read.csv(  paste0(data_dir, "prism_point_reyes_87_18.csv") )
enso        <- read.csv(  paste0(data_dir, "enso_data.csv") )
germ        <- read.csv( paste0(data_dir, "seedbaskets.csv") ) %>% 
                 select(g0:g2) %>% 
                 colMeans
germ_adj    <- read.csv( paste0(data_dir, "germ_adj.csv") )
fit         <- readRDS( paste0(data_dir, 'lupine_allvr_annual_anom.rds') )

# set up 8000 alternative simulations
job_df      <- expand.grid( clim_x  = c(-0.45,2),
                            loc_v   = c('BR (6)', 'BS (7)', 'DR (3)', 'NB (2)'),
                            sim_id  = paste0( 1   + seq(0, 900, length.out=10),':',
                                             100 + seq(0, 900, length.out=10) ),
                            post_id = 1:100,
                            stringsAsFactors = F ) %>% 
                  mutate( id    = 1:nrow(.) ) %>% 
                  separate( sim_id, c('start','end'), sep=':') %>% 
...
print('cons format')
cons_df    <- read.csv( paste0(data_dir, '/consumption.csv') ) %>%
...
sim_df    <- expand.grid( sim_i  = job_df$start:job_df$end,
                          clim_x = job_df$clim_x,
                          loc_v  = job_df$loc_v,
                          stringsAsFactors = F )
