...
# models ---------------------------------------------------------
mod_sl1  <- glmer(surv_t1 ~ log_area_t0 + log_area_t02 + log_area_t03 + 
                    tmp_tm1 + 
...
mod_s1   <- glmer(surv_t1 ~ log_area_t0 + log_area_t02 + log_area_t03 + 
                    tmp_tm1 + 
                  (1 | year) + (0 + log_area_t0 | year) + 
                  (1 | location) + (0 + log_area_t0 | location),
...
mod_fl1  <- glmer(flow_t0 ~ log_area_t0 + 
                    tmp_tm1 + 
                  (1 | year) + (0 + log_area_t0 | year) +
                  (1 | location) + (0 + log_area_t0 | location),
                  data=flow, family='binomial')
mod_fr1  <- glmer(numrac_t0 ~ log_area_t0 + 
                    tmp_tm1 + 
                  (1 | year) + (0 + log_area_t0 | year) + 
                  (1 | location) + (0 + log_area_t0 | location), 
...
# with interactions
mod_sl   <- glmer(surv_t1 ~ log_area_t0 + log_area_t02 + log_area_t03 + 
                    tmp_tm1 + tmp_tm1:location + 
                  (1 | year) + (0 + log_area_t0 | year) + 
                  (1 | location) + (0 + log_area_t0 | location),
                  data=seedl, family='binomial')
...
update_par_spec <- function(year_n, vr, loc_n){
  ...
  get_loc <- function(mod_obj,loc_n){
    ranef(mod_obj)$location %>% 
      ...
      select(-location) %>% 
      mutate( tmp_tm1 = ifelse(loc_n == 'AL (1)',
                               0,
                               fixef(mod_obj)[paste0('tmp_tm1:location',
                                              loc_n)])
              )
  }
  ...
  if( vr == 'surv_sl'){
    ...
    pars_yr$surv_sl_clim <- (pars_yr$surv_sl_clim + get_loc(mod_sl,loc_n)['tmp_tm1']) %>% as.numeric
  ...
  if( vr == 'surv'){
    ...
    pars_yr$surv_clim  <- (pars_yr$surv_clim + get_loc(mod_s,loc_n)['tmp_tm1']) %>% as.numeric
  }
  ...
  if( vr == 'flow'){
    ...
    pars_yr$flow_clim  <- (pars_yr$flow_clim + get_loc(mod_fl,loc_n)['tmp_tm1']) %>% as.numeric
  }
  ...
  if( vr == 'fert'){
    ...
    pars_yr$fert_clim  <- (pars_yr$fert_clim + get_loc(mod_fr,loc_n)['tmp_tm1']) %>% as.numeric
  }
...
# visual checks
update_par_spec(2011,'all',"AL (1)")$surv_sl_clim
...
ggplot(lam_s_df, aes(climate_anomaly, lam_s) ) +
  ...
  ggsave('results/ipm/lambdaS_vs_siteXclim_clim.tiff',
