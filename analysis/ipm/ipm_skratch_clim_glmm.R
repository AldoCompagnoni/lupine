...
# produce predictions
x_seq     <- seq(min(seedl$log_area_t0),
                 max(seedl$log_area_t0), length.out=100 )
...
# plot year spefici predictions
plot_year_pred <- function(year_v){
  y_pred    <- predict(mod_sl, newdata=subset(new_df,
                                             year==year_v) )
  lines(x_seq,boot::inv.logit(y_pred),lwd=1,col='grey')
}
lapply(2009:2018,plot_year_pred)



# survival adult plants
plot_binned_prop(surv, 10, log_area_t0, surv_t1)

# produce predictions
x_seq     <- seq(min(surv$log_area_t0),
                 max(surv$log_area_t0), length.out=100 )
new_df <- expand.grid( log_area_t0 = x_seq,
                       year        = 2006:2018) %>% 
            mutate( log_area_t02 = log_area_t0^2,
                    log_area_t03 = log_area_t0^3,
...
# plot year spefici predictions
plot_year_pred <- function(year_v){
  y_pred    <- predict(mod_s, newdata=subset(new_df,
                                             year==year_v) )
  lines(x_seq,boot::inv.logit(y_pred),lwd=1,col='grey')
}
lapply(2006:2018,plot_year_pred)
...
x_seq     <- seq(min(flow$log_area_t0),
                 max(flow$log_area_t0), length.out=100 )
new_df <- expand.grid( log_area_t0 = x_seq,
                       year        = 2006:2018) %>% 
            mutate( log_area_t02 = log_area_t0^2,
                    log_area_t03 = log_area_t0^3,
...
# plot year spefici predictions
plot_year_pred <- function(year_v){
  y_pred    <- predict(mod_fl, 
                       newdata=subset(new_df,year==year_v))
  lines(x_seq,boot::inv.logit(y_pred),lwd=1,col='grey')
}
lapply(2006:2018,plot_year_pred)
...
# plot year spefici predictions
plot_year_pred <- function(year_v){
  y_pred    <- predict(mod_fr, 
                       newdata=subset(new_df, year==year_v),
                       type='response')
  lines(x_seq,y_pred,lwd=1,col='grey')
}
lapply(2006:2018,plot_year_pred)
...
update_par_spec <- function(year_n, vr){
...
lam_stoch <- function(tmp_anom,vr){

  # store kernels
  ker_l     <- list()
  for(ii in seq_along(yrs)){
    ker_l[[ii]] <- kernel(tmp_anom,update_par_spec(yrs[ii],vr))
  }
...
lam_s_vec <- sapply(clim_x, lam_stoch, 'all')
lam_s_df  <- data.frame( climate_anomaly = clim_x,
                         lam_s           = lam_s_vec)
...
ggplot(lam_s_df, aes(climate_anomaly, lam_s) ) +
  ...
  ggsave( 'results/ipm/lambdaS_vs_clim.tiff',
