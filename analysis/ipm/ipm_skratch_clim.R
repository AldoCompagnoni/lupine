...
# models ---------------------------------------------------------
mod_sl   <- glm(surv_t1 ~ log_area_t0 + log_area_t02 + log_area_t03 + tmp_tm1,
                data=seedl, family='binomial')
mod_g_sl <- lm( log_area_t1 ~ log_area_t0, data=sl_grow)
mod_s    <- glm(surv_t1 ~ log_area_t0 + log_area_t02 + log_area_t03 + tmp_tm1, 
                  data=surv, family='binomial')
mod_g    <- lm( log_area_t1 ~ log_area_t0,
                data=grow)
mod_g2   <- lm( log_area_t1 ~ log_area_t0, data=grow) 
g_lim    <- range( c(grow$log_area_t0, grow$log_area_t1) )
mod_fl   <- glm(flow_t0 ~ log_area_t0 + tmp_tm1,
                  data=flow, family='binomial')
mod_fr   <- glm(numrac_t0 ~ log_area_t0 + tmp_tm1, 
...
# store pars (I do this for confidence intervals, below)
# not a good idea, plotting conf.int. was a shortcut anyway
store_pars <- function(){
  
  # store parameters in parent environmemt 
  # variable "pars_mean"
...
# store the parameters in pars_mean
store_pars()
...
mean_l  <- Re(eigen(ker)$value[1])

write.csv(data.frame( mean_lambda = mean_l), 
          'results/lambda_mean.csv',
          row.names=F)
...
ggplot(df, aes( x, y) ) +
  ...
  ggsave( 'results/ipm/det_lambda_vs_clim.tiff',
          width = 6.3, height = 6.3, compression="lzw" )

  
# # upper-lower conf intervals of lambda ---------------
# # NOT A GOOD IDEA :) :) :)
# 
# coef(mod_sl) %>% class
# cia = confint(mod_sl)
...
# form_ci <- function(x, up_low){
#   out <- confint(x) %>% 
#     as.data.frame %>% t %>% 
#     as.data.frame %>% .[up_low,] 
#   
#   out %>% as.numeric %>% setNames( names(out) )
#   
# }
#  
# # vital rate models 
# surv_sl_p <- form_ci(mod_sl,1)
# grow_sl_p <- form_ci(mod_g_sl,1)
# grow_sl_p <- c(grow_sl_p, summary(mod_g_sl)$sigma)
# surv_p    <- form_ci(mod_s,1)
# grow_p    <- form_ci(mod_g,1) 
# grow_p    <- c(grow_p, summary(mod_g)$sigma)
# flow_p    <- form_ci(mod_fl,1)
# fert_p    <- form_ci(mod_fr,1)
# size_sl_p <- sl_size
# fr_rac_p  <- coef(fr_rac) %>% exp
# seed_fr_p <- coef(seed_fr) %>% exp
# germ_p    <- germ_coef
# 
# # lower lambda (unrealistically low)
# # conf. int. are not "joint", doesn't make sense to use them.
# store_pars()
# ker     <- kernel(0,pars_mean)
# Re(eigen(ker)$value[1])
# 
# 
# # vital rate models 
# surv_sl_p <- form_ci(mod_sl,2)
# grow_sl_p <- form_ci(mod_g_sl,2)
# grow_sl_p <- c(grow_sl_p, summary(mod_g_sl)$sigma)
# surv_p    <- form_ci(mod_s,2)
# grow_p    <- form_ci(mod_g,2) 
# grow_p    <- c(grow_p, summary(mod_g)$sigma)
# flow_p    <- form_ci(mod_fl,2)
# fert_p    <- form_ci(mod_fr,2)
# size_sl_p <- sl_size
# fr_rac_p  <- coef(fr_rac) %>% exp
# seed_fr_p <- coef(seed_fr) %>% exp
# germ_p    <- germ_coef
# 
# # upper lambda: very high 
# # error
# store_pars()
# ker     <- kernel(0,pars_mean)
# Re(eigen(ker)$value[1])
