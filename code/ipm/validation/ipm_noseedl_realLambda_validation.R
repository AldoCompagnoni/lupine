...
# for(ii in 17:nrow(site_all)){
  ...
  list( lam_det = Re(eigen(ker)$value[1]),
        ...
        germ_est= germ_est,
        sb      = sb,
        obs_n   = subset(pop_n, year == site_all$year[ii]+1)$n,
        sim_n   = sum(ker %*% (freq_v)),
        det_n   = subset(pop_n, year == site_all$year[ii])$n * 
                  Re(eigen(ker)$value[1]) 
        )
...
lam_df   <- lapply(1:nrow(site_all), yr_lambdas, 
                   germ_est=T, sb=F) %>% bind_rows

# !!! lam_r_s is better at approximating long term lambda!!!
select(lam_df[1:7,],lam_r,lam_det,lam_r_s) %>% 
  apply(2,log) %>% 
  apply(2,mean) %>% exp

select(lam_df[10:17,],lam_r,lam_det,lam_r_s) %>% 
  apply(2,log) %>% 
  apply(2,mean) %>% exp

select(lam_df[18:23,],lam_r,lam_det,lam_r_s) %>% 
  apply(2,log) %>% 
  apply(2,mean) %>% exp


matplot(select(lam_df[1:7,],obs_n,sim_n,det_n),type='l',
