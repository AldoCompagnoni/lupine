...
climate_mods <- list(
  ...
  # precipitation
  numrac_t1 ~ log_area_t1 + ppt_t0 + (log_area_t1 | year) + (log_area_t1 | location) + (1|newid),
  numrac_t1 ~ log_area_t1 + ppt_tm1 + (log_area_t1 | year) + (log_area_t1 | location) + (1|newid),
  numrac_t1 ~ log_area_t1 + ppt_t0_tm1 + (log_area_t1 | year) + (log_area_t1 | location) + (1|newid),
  numrac_t1 ~ log_area_t1 + ppt_t0_tm2 + (log_area_t1 | year) + (log_area_t1 | location) + (1|newid),
  
  # temperature
  numrac_t1 ~ log_area_t1 + tmp_t0 + (log_area_t1 | year) + (log_area_t1 | location) + (1|newid),
  numrac_t1 ~ log_area_t1 + tmp_tm1 + (log_area_t1 | year) + (log_area_t1 | location) + (1|newid),
  numrac_t1 ~ log_area_t1 + tmp_t0_tm1 + (log_area_t1 | year) + (log_area_t1 | location) + (1|newid),
  numrac_t1 ~ log_area_t1 + tmp_t0_tm2 + (log_area_t1 | year) + (log_area_t1 | location) + (1|newid),
  
  # enso
  numrac_t1 ~ log_area_t1 + oni_t0 + (log_area_t1 | year) + (log_area_t1 | location) + (1|newid),
  numrac_t1 ~ log_area_t1 + oni_tm1 + (log_area_t1 | year) + (log_area_t1 | location) + (1|newid),
  numrac_t1 ~ log_area_t1 + oni_t0_tm1 + (log_area_t1 | year) + (log_area_t1 | location) + (1|newid),
  numrac_t1 ~ log_area_t1 + oni_t0_tm2 + (log_area_t1 | year) + (log_area_t1 | location) + (1|newid)
...
write.csv(out, 'results/ml_mod_sel/fert/fert_mod_sel_no2.csv', row.names=F)
...
write.csv(out_df, 
          'results/ml_mod_sel/fert/fert_best_mod_no2.csv',
          row.names=F)
...
plot(numall_t1 ~ log_area_t1, data=fert_clim,
     ylab = 'n. of racemes')
...
rep_df  <- fert_clim %>% 
              ...
              mutate( n_i = round(f_n/tot,1),
                      ...
                      log_notab_t1 = 4) %>% 
              as.data.frame %>% 
              select(year,location, log_notab_t1, n_i, tot ) #%>% 
              # mutate( tot = paste0( 'rep:',tot),
              #         n_i = paste0( 'fl/indiv.:',n_i) ) 

fert_plot <- fert_clim %>% 
                mutate( log_notab_t1 = log(notab_t1) )
...
ggplot(data  = fert_plot, 
  ...
  theme_bw() +
  ggtitle("Fertility (stage_t0 <> SL, area_t0 <> 0, numrac_t0 <>0, flow_t0 = 1)") + 
  geom_text(
    data    = rep_df,
   ...
   geom_text(
    data    = rep_df,
    mapping = aes(x = -Inf, y = log_notab_t1, label = tot),
    hjust   = -0.1,
    vjust   = -1
  ) +
