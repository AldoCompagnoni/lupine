...
# stage: long format
stage_long  <- long_data(stage, "stage", grep("STAGE",names(stage), value=T) ) %>%
...
size_long   <- size %>% 
                  ...
                  long_data("size", c('BRANCHES05', 'BRANCHES06', 'BRANCHES07',"area08") ) %>%
...
fecu_long   <- long_data( flow,  "racemes", grep('NUMRAC',names(flow),value=T) ) %>%
...
trans_pop_nb <- inner_join(demo_t0, demo_t1) %>%
                  ...
                  mutate( flow_t0 = replace(flow_t0, 
                                            !(stage_t0 == "R" | stage_t0 == "NR"), 
                                            NA),
                          flow_t1 = replace(flow_t1, 
                                            !(stage_t1 == "R" | stage_t1 == "NR"), 
                                            NA) ) %>%
